    ...
    def sample(self, size=1, err=0.1):
        ...
        if err is None:
            raise ValueError('An argument must be specified to the err '
                             'attribute in order to sample from the timeseries')
...
class TimeSeriesGroup(object):
    ...
    def __getitem__(self, feature):
        if isinstance(feature, list):
            for i in feature:
                if i not in self.features:
                    raise ValueError("feature '{}' not in features".format(self.features))
        else:
    ...
    def dtw_matrix(self):
        ...
        :return:
        """
        from timeseries.dtw import FastDTW
