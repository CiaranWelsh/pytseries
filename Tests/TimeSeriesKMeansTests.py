...
class TimeSeriesKMeansTests(unittest.TestCase):
    def setUp(self):
        self.data_file = os.path.join(dire, 'MeanMicroarrayDEGS_gt_75.csv')
    ...
    def test_convert_feature_labels(self):
        tsg = TimeSeriesGroup(self.data.iloc[:30])
        k = TimeSeriesKMeans(tsg, n_clusters=5, metric='dtw', random_state=1)
        labels = k.convert_labels_to_features()
        self.assertEqual(labels.loc['CTGF'].values[0], 0)
    ...
    def test_plot(self):
        ...
        for i in figs:
            self.assertTrue(isinstance(figs, Figure))

    def test_format_inertoia(self):
        tsg = TimeSeriesGroup(self.data[:50])
        k = TimeSeriesKMeans(tsg, n_clusters=5, metric='dtw', random_state=1)
        k.format_inertia()
